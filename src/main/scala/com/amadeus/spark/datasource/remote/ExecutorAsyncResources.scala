...
object ExecutorAsyncResources extends Logging {
  ...
  /**
   * Creates the ExecutorService for async operations.
   *
   * Configurable thread pool size with sensible defaults for I/O bound work.
   * Threads are named and set as daemon for better debugging and graceful shutdown.
   *
   * @param config configuration map containing thread pool settings
   */
  private def createExecutorService(config: java.util.Map[String, String]): Unit = {
    ...
    logInfo(s"[EXECUTOR-RESOURCES] Initializing async resources with $threads threads.")
    ...
    executorService = Executors.newFixedThreadPool(
      ...
      new ThreadFactory {
        ...
        override def newThread(runnable: Runnable): Thread = {
          ...
          thread.setName(s"spark-remote-file-async-${counter.incrementAndGet()}")
    ...
    logInfo(s"[EXECUTOR-RESOURCES] Initialized executorService successfully with $threads threads")
  }

  /**
   * Creates the shared HttpClient instance with connection pooling.
   *
   * @param config configuration map containing timeout settings
   */
  private def createHttpClient(config: java.util.Map[String, String]): Unit = {
    ...
    logInfo(s"[EXECUTOR-RESOURCES] Initializing shared HttpClient with ${connectionTimeoutMs}ms connection timeout")
